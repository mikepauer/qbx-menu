...
const openMenu = (data = null) => {
	let html = '';	
	if (data[0].enableSearch) {
		html += `<div class="search-container"><input type="text" id="search-input" placeholder="Search..."></div>`;
	}
    ...
    data.forEach((item, index) => {
        if (!item.hidden) {
            ...
            if (item.params) {
				buttonParams[index] = item.params;
			} else {
					buttonParams[index] = false;
			}
			if (item.image) {
